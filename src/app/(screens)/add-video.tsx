...
export default function AddVideo() {
  ...
  // Hook para pegar thumbnail automaticamente
  const thumbnail = useYouTubeThumbnail(url, "hq");

  async function handleAddVideo() {
    if (!url) {
      Alert.alert("Erro", "Preencha uma URL válida do YouTube!");
      return;
    }
    if (!title) {
      Alert.alert("Erro", "Preencha o título do Vídeo!");
      return;
    }
    if (category === "") {
      Alert.alert("Erro", "Selecione a categoria do Vídeo!");
      return;
    }
    if (!thumbnail) {
      Alert.alert("Erro", "Thumbnail não encontrada!");
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.pickerWrapper}>
          ...
          >
            ...
            <Picker.Item label="Geral" value="geral" />
            <Picker.Item label="Culto Gravado" value="cultos" />
          </Picker>
        </View>
